...
直接编译生产限制核心、上下文、守卫和 Harmony 补丁，当前 **120 项**。覆盖普通双人、单人、人格排泄、日常/仪式、实际主人最高许可、装备/特化、玩家命令和 AI 候选、精确清理、存档及对象池复用。
...
该模式把基因替身移入其他命名空间，让生产动态发现返回缺席；跳过四个需要实际基因类型的行为用例，其余 **116 项**仍运行真实 PatchAll。省略 Harmony 参数则运行相同生产逻辑的直接调用诊断模式。
